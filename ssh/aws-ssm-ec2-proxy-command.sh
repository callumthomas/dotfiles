...
# Redirect all non-essential output to stderr so only the SSM session stream
# reaches stdout — tunnel clients (e.g. DataGrip) break if anything else leaks through
...
REGION_SEPARATOR='--'
# Support instance_id--region format (e.g. i-abc123--eu-west-1)
...
echo "Add public key ${ssh_public_key_path} for ${ssh_user} at instance ${instance_id} for 30 seconds"
ssh_public_key="$(cat "${ssh_public_key_path}")"
aws ssm send-command \
  --instance-ids "${instance_id}" \
  --document-name 'AWS-RunShellScript' \
  --comment "Add an SSH public key to authorized_keys for 30 seconds" \
  --parameters commands="
  \"
    set -eu
...
echo "Start ssm session to instance ${instance_id}"
# Restore stdout for the actual SSH session
exec 1>&3
